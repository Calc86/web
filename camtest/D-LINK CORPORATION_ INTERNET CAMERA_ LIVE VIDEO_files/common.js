...
g_isIPv6 = isIPv6(location.host);
//Initial IPv6
var tempurl = location.host.split(":");
if(g_isIPv6)
{
	//alert(location.host);
	if(tempurl.length==9)//IP[8] + Port[1]
	{
		g_host= tempurl[0];
		for(var i=1; i<9; i++)
			g_host+= (":"+tempurl[i]);
...
//
      ...
      //Status 0 is none, 1 is Privacy Mask , 2 is motion ,3 is Zoom, 4 is focus in/out, 5 is ???, 6 is ???, 7 is Motion dyna
      obj.Status(mode);
...
{
  //http://192.168.0.248/vb.htm?paratest=profile1eptzcoordinate
...
function OnSubmitReadyStateProcessPublic()
{
  if (g_SubmitHttpPublic.readyState==4)
  {
    ...
    {
      alert(g_SubmitHttpPublic.status+" "+ GL("err_submit_fail"));
      WS(GL("fail_"));
    }
    else
    {
...
function GetHtmlHeaderNoBodyStr(extTitle,otherName,refreshTime)
{
  ...
  if(g_isSupportEPTZ == 1)
    o+='<link rel="stylesheet" type="text/css" media="screen" href="/uvumi/uvumi-crop.css" />';	
...
function WriteBottom(lastctx)
{
  ...
  o+='<div class="div1" align="center">Copyright &copy; 2011 D-Link Corporation.</div><br>';
  o+=GetMapLinkStr();
  if (lastctx != null)
  {
    o+=lastctx;
...
//alert(g_MaxSubmitLen+"::::"+len);
      if (len < 10 )
      {
        //send submit
//alert(o);
        SendHttp(o,isAsync,SubmitCallBack);
        if (g_httpOK)
        {
          o = c_iniUrl;
          len = g_MaxSubmitLen;
...
function Ctrl_SelectEx1(id,itemlist,valuelist,val,setcmd,onChangeFunc,checker,inactive)
{
  ...
  this.onChangeFunc = onChangeFunc;
  this.checker = checker;
  this.html = SelectObjectNoWriteEx(this.id,this.itemlist,this.valuelist,this.value,this.onChangeFunc);
  this.GV = function (){ return GetValue(this.id); };
  this.SV = function (val){ SetValue(this.id,val); };
  this.IsPass = function (isQuite){ return CPASS(this,isQuite);};
