...
class cam {
    ...
    protected $name = '';
    protected $cam_ip = '';
    protected $source_proto = '';
    protected $source_port = '';
    protected $source_path = '';
    protected $stream_port = '';
    ...
    public function __construct($id) {
        ...
        $q = "select";
        $q.= " `cam-ip`,`source-proto`,`source-port`,`source-path`,";
        $q.= " `cam-name`,`stream-port`,`stream-path` ";
        $q.= " from cam where id=$this->id";
        ...
        if ($n) {
            ...
            //source
            $this->cam_ip = $row['cam-ip'];
            $this->source_proto = $row['source-proto'];
            $this->source_port = $row['source-port'];
            $this->source_path = $row['source-path'];

            //srv
            $this->name = $row['cam-name'];
            $this->stream_port = $row['stream-port'];
            $this->stream_path = $row['stream-path'];
        }
    }
    ...
    protected function get_srv_host() {
        //return '10.112__28.35';
        return LIVE_HOST;
    }

    public function src_srv() {
        return $this->url(
                        $this->get_srv_proto(), $this->get_srv_host(), $this->stream_port, $this->stream_path . '.mp4'
        );
    }

    public function src_source() {
        return $this->url(
                        $this->source_proto, $this->cam_ip, $this->source_port, $this->source_path
        );
    }
    ...
    public function src_srv_rtsp() {
        ...
        return $this->url(
                        'rtsp', $this->get_srv_host(), '1' . $this->stream_port, $this->stream_path . '.mp4'
        );
    }
    ...
    protected function vlc_plugin($source) {
        ...
        $ret.= '<center>
            <embed type="application/x-vlc-plugin" pluginspage="http://www.videolan.org"
               version="VideoLAN.VLCPlugin.2"  width="' . $this->w . '"  height="' . $this->h . '" id="vlc_' . $this->id . '" loop="yes" autoplay="yes" 
    ...
    protected function vlc2_plugin($source) {
        ...
        $ret.= '<center>
            <OBJECT classid="clsid:9BE31822-FDAD-461B-AD51-BE1D1C159921"
                 ...
                 width="' . $this->w . '" height="' . $this->h . '" id="vlc'.rand().'" events="True">
               <param name="Src" value="'.$source.'" />
               <param name="ShowDisplay" value="True" />
               <param name="AutoLoop" value="True" />
               <param name="AutoPlay" value="True" />
               <embed id="vlcEmb'.rand().'"  type="application/x-google-vlc-plugin" version="VideoLAN.VLCPlugin.2" autoplay="yes" loop="yes" width="'.$this->w.'" height="'.$this->h.'"
                 target="'.$source.'" ></embed>
            </OBJECT>
               </center>';
        $ret.= '</div>';
